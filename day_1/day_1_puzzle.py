...
To begin, get your puzzle input (day_1_puzzle_input_1.txt).
'''

# Read Input from file to string. 
# Use string methods to split the numbers while discarding spaces and \n. 
# Pass it all to a list.
# Even numbers go on list 1, odd numbers go on list 2.
...
file = open('.\\day_1\\day_1_puzzle_input.txt')
raw_input = file.read().split()
raw_list_1 = []
raw_list_2 = []
for x in range(len(raw_input)):
    if x == 0 or x % 2 == 0:
        raw_list_1.append(raw_input[x])
    else:
        raw_list_2.append(raw_input[x])
raw_list_1.sort()
raw_list_2.sort()
list_1 = [int(x) for x in raw_list_1]
list_2 = [int(x) for x in raw_list_2]
...
for x in list_1:
    ...
    if number_of_repetitions>0:
        similarity_score += x * number_of_repetitions
